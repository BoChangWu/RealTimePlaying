...
class ATTN(tf.keras.layers.Layer):
    def __init__(self,n_state,n_head,seq):
        ...
        self.n_sate = n_state * 3
        self.n_head = n_head

        #todo:
        E_initializer = tf.constant_initializer(0)
        self.E = tf.Variable(E_initializer(shape=[16,seq,32],dtype=tf.float32),name="E")


    def split_heads(self,x):
        # from [batch,sequence,feature] to [batch,heads,sequence,features]
...
class WTE(tf.keras.layers.Layer):
    def __init__(self, n_vocab, n_embd):
        ...
        intializer = tf.random_normal_initializer(stddev=0.02)
        self.wte = tf.Variable(intializer([n_vocab, n_embd]), name="wte")
    
    def call(self, inputs):
        return [tf.gather(self.wte,inputs),self.wte]

def dilated_causal_Conv1D(dilation_r,activation_fn,dilated_num=0, num_stack=0):
    return Conv1D(filter=512,
    kernel_size=2,
    strides=1,
    padding="causal",
...
def TransformerGenerator(hparams,input_shape):
    ...
    for layer in range(n_layer):
        ...
        a = MyConv1d(nx, nx*3, [batch_size, n_sequence])(nor)
        ...
        ## MLP ##
        nor = NormalizeDiagonal(n_embd)(h)
        a = MyConv1d(nx, nx*4, [batch_size, n_sequence])(nor)
        a = Activation('gelu')(a)
        m = MyConv1d(nx*4, nx, [batch_size,n_sequence])(a)
        ###
        h= Add()([h,m])
        ###
...
def TransformerDiscriminator(hparams, inputs_shape):
    ...
    n_sequence = hparams['Time']
    batch_size = 1
    inputs = Input(shape=inputs_shape,dtype=tf.float32)
    ...
    for layer in range(n_layer):
        ...
        a = MyConv1d(nx, nx * 3, [batch_size, n_sequence])(nor)
        a, present = ATTN(nx, n_head, n_sequence)(a)
        a = MyConv1d(nx, nx, [batch_size, n_sequence])(a)
        ###
        h = Add()([h,a])
        ###
        ## MLP ##
